...
from ..build.lib import sync_bn_gpu, sync_bn_cpu
...
class moments(Function):
    ...
    def forward(ctx, x):
        if x.is_cuda:
            ex, ex2 = sync_bn_gpu.expectation_forward(x)
        else:
            raise NotImplemented
        return ex, ex2

    @staticmethod
    def backward(ctx, x, dex, dex2):
        if x.is_cuda:
            dx = sync_bn_gpu.expectation_backward(x, dex, dex2)
        else:
            raise NotImplemented
        return dx
...
class syncbatchnorm_(Function):
    ...
    def forward(cls, ctx, x, gamma, beta, running_mean, running_var,
        ...
        if ctx.training:
            if x.is_cuda:
                _ex, _exs = sync_bn_gpu.expectation_forward(x)
            else:
                raise NotImplemented
        ...
        if x.is_cuda:
            ...
            y = sync_bn_cpu.batchnorm_forward(x, _ex, _exs, gamma, beta, ctx.eps)
    ...
    def backward(ctx, dz):
        ...
        if dz.is_cuda:
            dx, _dex, _dexs, dgamma, dbeta = \
                sync_bn_gpu.batchnorm_backward(dz, x, _ex, _exs, gamma, beta, ctx.eps)
        else:
            raise NotImplemented
        ...
        if ctx.training:
            ...
            if x.is_cuda:
                dx_ = sync_bn_gpu.expectation_backward(x, _dex, _dexs)
            else:
                raise NotImplemented
            dx = dx + dx_
...
def _act_forward(ctx, x):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            sync_bn_gpu.leaky_relu_forward(x, ctx.slope)
        else:
            raise NotImplemented
    else:
...
def _act_backward(ctx, x, dx):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            sync_bn_gpu.leaky_relu_backward(x, dx, ctx.slope)
        else:
            raise NotImplemented
    else:
        assert ctx.activation == 'none'
...
class inp_syncbatchnorm_(Function):
    ...
    def forward(cls, ctx, x, gamma, beta, running_mean, running_var,
        ...
        # BN forward + activation
        if x.is_cuda:
            sync_bn_gpu.batchnorm_inp_forward(x, _ex, _exs, gamma, beta, ctx.eps)
        else:
            raise NotImplemented
    ...
    def backward(ctx, dz):
        ...
        if dz.is_cuda:
            dx, _dex, _dexs, dgamma, dbeta = \
                sync_bn_gpu.batchnorm_inp_backward(dz, z, _ex, _exs, gamma, beta, ctx.eps)
        else:
            raise NotImplemented
        ...
        if ctx.training:
            ...
            if z.is_cuda:
                sync_bn_gpu.expectation_inp_backward(dx, z, _dex, _dexs, _ex, _exs, gamma, beta, ctx.eps)
            else:
                raise NotImplemented
